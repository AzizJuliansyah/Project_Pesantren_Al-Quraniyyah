(function($) {
  ...
  $(function() {
    if ($("#performanceLine").length) {
      ...
      // Ambil data alumni per angkatan melalui AJAX
      $.ajax({
        url: '/get-alumni-data',
        method: 'GET',
        success: function (response) {
          var labels = response.map(item => `Angkatan ${item.angkatan}`); // Tambahkan kata 'Angkatan'
          var dataAlumni = response.map(item => item.alumni_count); // Data total alumni
